...
int socketServerInit(unsigned int myport, unsigned int lisnum, char * serveraddr,int *socketID)
{
    ...
    egsip_log_info("port=%d lisnum=%d\n",myport,lisnum);
    if (strlen(serveraddr)!=0){  
		egsip_log_info("addr=%s\n",serveraddr);
        my_addr.sin_addr.s_addr = inet_addr(serveraddr);
    }
    else{  
        my_addr.sin_addr.s_addr = INADDR_ANY; 
		egsip_log_info("addr is INADDR_ANY\n");
    }
          
    if (bind(sockfd, (struct sockaddr *) &my_addr, sizeof(struct sockaddr))== -1)   
    {  
        egsip_log_error("socket bind error!");  
        return -1;  
    }  
      
    if (listen(sockfd, lisnum) == -1)   
    {  
        egsip_log_error("socket listen error");  
        return -1;  
    }
	*socketID = sockfd;
...
int socketServerLoopRsv(int sockfd)
{
    ...
    {  
        ...
        while (1){  
            ...
            if (retval == -1)   
            {  
                egsip_log_error("select error, will disconnect! errno=%d[%s]\n",errno,strerror(errno)); 
                break;  
            }   
            else if (retval == 0)   
            {  
            ...
            { 
                ...
                {  
                    ...
                    if (len > 0){  
                        egsip_log_debug("Rsv msg success:'%s'，共%d个字节的数据\n",buf, len);
						msg_struct msgs;
                    ...
                    {  
                        if (len < 0)  
                            egsip_log_error("Rsv msg fail!errno=%d，errmsg='%s'\n",errno, strerror(errno));  
                        else  
                            egsip_log_error("client disconnected...\n");  
                        break;  
                    }  
                }  
            }  
...
int socketServerLoopSend()
{
	...
	while(1){
		...
		if(ret<0){
			egsip_log_error("socketServerLoopSend dequeue failed!\n");
		...
		if (len > 0)
			egsip_log_info("msg:%s\tsend success，len = %d bytes！\n", buf, len);
		else{
			egsip_log_info("msg'%s'send fail！error=%d，errinfo='%s'\n",buf, errno, strerror(errno));
			sleep(SOCKET_SEND_SLEEP_SEC);
			continue;
		} 
	}
...
int socketServerLoopSendShort()
{
	...
	while(1){
		...
		if(ret<0){
			egsip_log_error("socketServerLoopSendShort dequeue failed!\n");
			sleep(SOCKET_SEND_SLEEP_SEC);
			continue;
		}
